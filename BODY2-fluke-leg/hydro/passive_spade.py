#!/usr/bin/env python3
"""现有铲形桨头（鱼尾平面形 44×42）颈部扭簧铰、头朝后拖：被动俯仰 + 平行四边形竖直沉浮：流固耦合（基波相量法）
...
U = 0.25; RHO = 1000.0
import sweep_fluke as _sf
_sf.S_FIX = 0.044 * 0.042 * 1.4 / 2
GEO, B_SPAN, C_MID = planform("fishtail", 0.044**2 / _sf.S_FIX, 0.4)
C_TIP = GEO["stations"][0][1]          # 14.3 mm：uvlm_foil 的 pivot 参照弦
R_PAR = 0.039                           # 平行四边形长边 [m]
M_FOIL = 0.008                          # PLA 铲头 [kg]
X_CG = 0.6 * C_MID                      # 鱼尾形（后宽）质心偏后

def kin(f, h0, x_p, th0=0.0, psi=90.0):
    A_s = R_PAR * (1 - math.cos(math.asin(h0 / R_PAR))) / 2
    return dict(f=f, h0=h0, theta0=th0, psi=psi, pivot=x_p * 1e-3 / C_TIP, surge_amp=0.0, surge_phase=90.0)   # 两自由度逆解：E 走直线，无纵荡
...
def run(f, h0, x_p, th0, psi, **kw):
    ...
    T = float(r["T_mean"]); CT = T / (0.5 * RHO * U**2 * _sf.S_FIX) - 0.05
    T_v = CT * 0.5 * RHO * U**2 * _sf.S_FIX
...
if __name__ == "__main__":
    ...
    if stage == "ident":
        ...
        ks = [1e-3, 2e-3, 3e-3, 5e-3, 8e-3]
        jobs = [(f, h0, x_p, k, c) for f, h0 in ((1.5, 0.02), (1.5, 0.03), (2.0, 0.02), (2.0, 0.03)) for x_p in (-5.0, 0.0, 5.0)
                for c in (0.1e-3,) for k in ks]
        res = []
        with Pool(2) as pool:
        ...
        json.dump(res, open(os.path.join(HERE, "passive_spade_sweep.json"), "w"), indent=1, default=float)
